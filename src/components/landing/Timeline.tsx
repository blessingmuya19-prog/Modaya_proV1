...
export function Timeline() {
  ...
  return (
    <div style={{ borderTop: '1px solid #E6EBF5', background: '#F7F9FE', flexShrink: 0, height: 140 }}>
      {/* Header */}
      <div style={{ height: 32, borderBottom: '1px solid #E6EBF5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 10, color: '#7A869E', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Timeline</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#7A869E' }}>
        ...
        <span style={{ fontSize: 10, color: '#7A869E' }}>3:32 total</span>
      </div>
      ...
      <div style={{ position: 'relative', padding: '8px 8px 8px', height: 108, overflow: 'hidden' }}>
        ...
        <div style={{ paddingLeft: LABEL_W, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[
            { label: 'Video',    clips: videoClips,   h: 22, baseBg: '#E6EBF5', baseBorder: '#E6EBF5', aiBg: 'rgba(79,140,255,0.12)', aiBorder: 'rgba(79,140,255,0.4)' },
            { label: 'Audio',    clips: audioClips,   h: 16, baseBg: '#0F1B33', baseBorder: '#1a1a2a', aiBg: '', aiBorder: '' },
            ...
            <div key={ti} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <div style={{ position: 'absolute', left: -LABEL_W, width: LABEL_W - 4, textAlign: 'right', fontSize: 9, color: '#7A869E' }}>
                {track.label}
              </div>
              <div
                style={{ flex: 1, height: track.h, position: 'relative', background: '#EEF2FB', cursor: 'pointer', borderRadius: 3 }}
                onClick={e => {
                  ...
                  {ti === 1 && (
                    <div style={{ position: 'absolute', inset: 2, display: 'flex', alignItems: 'flex-end', gap: 0.5, overflow: 'hidden' }}>
                      {AUDIO_WAVEFORM.map((h, i) => (
                        <div key={i} style={{ flex: 1, borderRadius: 1, height: h, background: 'rgba(59,111,246,0.06)' }} />
                      ))}
                  </div>
                )}
              </div>
        ...
        {mockCuts.map(cut => (
          ...
          >
            ...
            <div style={{ position: 'absolute', top: -4, left: -4, width: 10, height: 10, background: 'rgba(200,255,61,0.85)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 5, color: '#F4F7FE', fontWeight: 900 }}>AI</span>
            </div>
            {hoveredCut === cut.id && (
              <div style={{
                position: 'absolute', bottom: '100%', left: '50%', transform: 'translateX(-50%)', marginBottom: 8,
                background: '#EEF2FB', border: '1px solid #E6EBF5', borderRadius: 8,
                padding: '8px 10px', width: 160, boxShadow: '0 4px 24px rgba(31,54,110,0.15)',
                pointerEvents: 'none', animation: 'fade-in 0.15s ease',
              }}>
                <p style={{ fontSize: 11, fontWeight: 600, color: '#0F1B33', margin: '0 0 3px' }}>AI removed {cut.duration}s</p>
                <p style={{ fontSize: 10, color: '#7A869E', margin: 0 }}>&ldquo;{cut.reason}&rdquo;</p>
              </div>
            )}
          </div>
        ))}
        ...
        {mockKeeps.map(keep => (
          ...
          >
            ...
            <div style={{ position: 'absolute', top: -4, left: -4, width: 10, height: 10, background: 'rgba(59,111,246,0.08)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 2 }} />
            {hoveredKeep === keep.id && (
              <div style={{
                position: 'absolute', bottom: '100%', left: '50%', transform: 'translateX(-50%)', marginBottom: 8,
                background: '#EEF2FB', border: '1px solid #E6EBF5', borderRadius: 8,
                ...
                <p style={{ fontSize: 10, color: '#7A869E', margin: 0 }}>&ldquo;{keep.reason}&rdquo;</p>
              </div>
            )}
          </div>
        ))}
