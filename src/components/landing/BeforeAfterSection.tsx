...
/* colour palette */
const C = {
  keep:  { bg: 'rgba(91,130,255,0.12)', border: 'rgba(91,130,255,0.28)', text: '#6E92FF' },
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: 8, padding: '9px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: kind === 'keep' ? '#B7C0D4' : 'rgba(248,113,113,0.55)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: kind === 'waste' ? 'line-through' : 'none' }}>{label}</span>
        <span style={{ fontSize: 11, color: kind === 'keep' ? '#5E6885' : 'rgba(248,113,113,0.35)', flexShrink: 0 }}>{dur}</span>
      </div>
    </div>
  );
}
...
function Panel({ title, label, clips, totalSec, maxSec, labelColor, badge, badgeBg, delay = 0 }:
  ...
  return (
    <div ref={ref} style={{ flex: '1 1 280px', minWidth: 0, opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(24px)', transition: `opacity 600ms ${delay}ms ease, transform 600ms ${delay}ms cubic-bezier(0.22,1,0.36,1)` }}>
      <div style={{ background: '#141A28', border: '1px solid #242C3E', borderRadius: 16, overflow: 'hidden', boxShadow: '0 8px 30px rgba(0,0,0,0.3)' }}>

        {/* Panel header */}
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #1F2739', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#8B95AD', letterSpacing: '0.04em', textTransform: 'uppercase' }}>{title}</span>
        ...
        {/* Duration bar */}
        <div style={{ padding: '14px 18px', borderTop: '1px solid #1F2739' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
            <span style={{ fontSize: 11, color: '#5E6885' }}>{label}</span>
  ...
  return (
    <section className="section-pad" style={{ maxWidth: 960, margin: '0 auto', paddingLeft: 24, paddingRight: 24 }}>
      ...
      <div ref={headRef} style={{ textAlign: 'center', marginBottom: 'clamp(32px,4vw,56px)', opacity: headVisible ? 1 : 0, transform: headVisible ? 'none' : 'translateY(20px)', transition: 'opacity 600ms ease, transform 600ms cubic-bezier(0.22,1,0.36,1)' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 14px', background: 'rgba(91,130,255,0.06)', border: '1px solid rgba(91,130,255,0.15)', borderRadius: 9999, marginBottom: 20 }}>
          <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#6E92FF' }}>Before & After</span>
        </div>
        <h2 style={{ fontFamily: "'Inter Tight',sans-serif", fontWeight: 700, fontSize: 'clamp(28px,4vw,48px)', letterSpacing: '-0.04em', color: '#F2F5FC', margin: '0 0 14px', lineHeight: 1.1 }}>
          Every edit, automated
        </h2>
        <p style={{ fontSize: 16, color: '#8B95AD', maxWidth: 480, margin: '0 auto', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }} className="before-after-row">
        ...
        <div ref={arrowRef} style={{
          ...
          <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(91,130,255,0.08)', border: '1px solid rgba(91,130,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ArrowRight size={16} color="#6E92FF" strokeWidth={2} />
          </div>
        </div>
      ...
      <div style={{ marginTop: 40, textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'baseline', gap: 6, padding: '10px 20px', background: 'rgba(91,130,255,0.05)', border: '1px solid rgba(91,130,255,0.12)', borderRadius: 12 }}>
          <span style={{ fontSize: 22, fontWeight: 800, color: '#6E92FF', fontFamily: "'Inter Tight',sans-serif", letterSpacing: '-0.04em' }}>67%</span>
          <span style={{ fontSize: 13, color: '#8B95AD' }}>shorter video — zero manual editing</span>
        </div>
      </div>
    </section>
  );
