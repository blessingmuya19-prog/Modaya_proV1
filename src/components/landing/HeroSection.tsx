...
export function HeroSection() {
  ...
  return (
    <section style={{ position:'relative', zIndex:0, background:'#050505', overflow:'hidden' }}>
      <style>{`
      ...
      <div style={{ position:'relative', zIndex:10, maxWidth:'min(860px,90vw)', margin:'0 auto', padding:'clamp(100px,14vw,160px) clamp(14px,4vw,32px) 0', textAlign:'center' }}>
        ...
        {uploadState === 'empty' && (
          <div style={{ animation:'slide-up 0.7s cubic-bezier(0.22,1,0.36,1) 0.18s both' }}>
            ...
            >
              <div style={{ width:48, height:48, borderRadius:'50%', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)', boxShadow:'inset 0 1px 0 rgba(255,255,255,0.1)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
                <Upload size={20} color="rgba(255,255,255,0.6)" strokeWidth={1.75} />
              </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'fade-up 0.45s cubic-bezier(0.22,1,0.36,1) both' }}>
            ...
            <div style={{ background:'#0A0A0A', border:`1px solid ${promptPhase !== 'idle' ? 'rgba(79,140,255,0.3)' : '#1e1e1e'}`, borderRadius:14, overflow:'hidden', boxShadow: promptPhase !== 'idle' ? '0 0 40px rgba(79,140,255,0.08)' : 'none', transition:'all 400ms ease', textAlign:'left' }}>
              <div style={{ padding:'16px 18px 12px', minHeight:96 }}>
                ...
                {promptPhase === 'typing' && <span style={{ fontSize:14, color:'#A1A1A1', lineHeight:1.65 }}>{demoTyped}<span style={{ display:'inline-block', width:2, height:14, background:'#4F8CFF', marginLeft:1, verticalAlign:'text-bottom', animation:'cursor-blink 0.9s step-end infinite' }} /></span>}
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 14px', borderTop:'1px solid #141414', background:'#050505' }}>
            ...
            {[...Array(2)].map((_, si) => (
              <div key={si} style={{ display:'flex', alignItems:'center', gap:48, paddingRight:48 }}>
                {BRAND_LOGOS.map(({ name, svg }) => (
                  <div key={name} style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0, opacity:0.25, color:'#FFFFFF', transition:'opacity 250ms ease', cursor:'default' }}
                  ...
                  >
                    ...
                    <span style={{ fontSize:14, fontWeight:700, color:'#FFFFFF', letterSpacing:'-0.01em', whiteSpace:'nowrap' }}>{name}</span>
                  </div>
                ))}
              </div>
            ))}
...
function EditorMockup({ isDone }: { isDone: boolean }) {
  return (
    <div style={{ position:'relative' }}>
      ...
      <div style={{ borderRadius:14, border:'1px solid #1a1a1a', overflow:'hidden', background:'#0A0A0A', boxShadow:'0 40px 100px rgba(0,0,0,0.9), 0 0 0 1px rgba(255,255,255,0.025)' }}>

        {/* Topbar */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', height:40, background:'#0f0f0f', borderBottom:'1px solid #1a1a1a' }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            <div style={{ width:44, height:22, background:'#181818', borderRadius:5, border:'1px solid #222' }} />
            <div style={{ width:44, height:22, background:'#181818', borderRadius:5, border:'1px solid #222' }} />
            <div style={{ width:56, height:22, background:'linear-gradient(135deg,#4F8CFF,#326FEA)', borderRadius:5 }} />
          </div>
        </div>
        ...
        <div style={{ display:'flex', height:420 }}>
          ...
          {/* AI Sidebar */}
          <div style={{ width:150, background:'#0A0A0A', borderRight:'1px solid #141414', padding:12, flexShrink:0, display:'flex', flexDirection:'column', gap:2 }}>
            <p style={{ fontSize:9, color:'#222', letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 8px' }}>✦ AI Edit</p>
            {[
              ...
              <div key={i} style={{ padding:'7px 9px', borderRadius:7, background: item.active ? 'rgba(79,140,255,0.1)' : 'transparent', border:`1px solid ${item.active ? 'rgba(79,140,255,0.22)' : 'transparent'}`, marginBottom:2 }}>
                <div style={{ fontSize:10, color: item.active ? '#FFFFFF' : '#282828', fontWeight: item.active ? 500 : 400, marginBottom:2 }}>{item.label}</div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#050505' }}>
            ...
            <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
              <div style={{ width:'100%', aspectRatio:'16/9', background:'#0d0d0d', borderRadius:10, border:`1px solid ${isDone ? 'rgba(79,140,255,0.2)' : '#181818'}`, position:'relative', overflow:'hidden', boxShadow: isDone ? '0 0 28px rgba(79,140,255,0.07)' : 'none', transition:'all 600ms ease' }}>
                ...
                {/* Fake scene lines */}
                <div style={{ position:'absolute', inset:'12% 8%', display:'flex', flexDirection:'column', gap:10, justifyContent:'center', opacity:0.06 }}>
                  {[75,55,85,45,70].map((w,i) => <div key={i} style={{ height:3, width:`${w}%`, background:'#fff', borderRadius:2 }} />)}
                </div>
                <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <div style={{ width:42, height:42, borderRadius:'50%', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <Play size={13} fill="white" color="white" style={{ marginLeft:2 }} />
                  </div>
                </div>
                <div style={{ position:'absolute', top:10, left:10, display:'flex', alignItems:'center', gap:4, background:'rgba(0,0,0,0.75)', padding:'2px 8px', borderRadius:9999 }}>
                ...
                <div style={{ position:'absolute', bottom:12, right:10 }}>
                  <span style={{ fontSize:9, color: isDone ? '#4F8CFF' : '#333', background: isDone ? 'rgba(79,140,255,0.1)' : 'rgba(0,0,0,0.5)', border: isDone ? '1px solid rgba(79,140,255,0.25)' : '1px solid #2a2a2a', padding:'1px 7px', borderRadius:9999, fontWeight:600, transition:'all 600ms ease' }}>
            ...
            <div style={{ background:'#0A0A0A', borderTop:'1px solid #141414', padding:'10px 16px 14px' }}>
              ...
              <div style={{ marginBottom:5 }}>
                ...
                <div style={{ display:'flex', gap:2, alignItems:'center', height:18 }}>
                  {[3,0.3,2,0.3,4,0.3,1.5,0.3,3,0.3,2].map((w,i) => {
                    ...
                    return <div key={i} style={{ height:isCut?8:18, width:w*28, borderRadius:isCut?1:3, flexShrink:0, alignSelf:isCut?'center':'stretch', background:isCut?'#141414':isKept&&isDone?'rgba(79,140,255,0.18)':'#181818', border:`1px solid ${isCut?'#1a1a1a':isKept&&isDone?'rgba(79,140,255,0.35)':'#222'}`, transition:'all 600ms ease', position:'relative' }}>
              ...
              <div style={{ display:'flex', gap:2, height:10, marginBottom:4 }}>
                {[4,3,3,2.5,2].map((w,i) => <div key={i} style={{ height:10, width:w*28, borderRadius:2, flexShrink:0, background:'#121212', border:'1px solid #1a1a1a' }} />)}
              </div>
              <div style={{ display:'flex', gap:2, height:7, marginBottom:9 }}>
                {[2,1.5,2,1,2.5,1.5].map((w,i) => <div key={i} style={{ height:7, width:w*28, borderRadius:2, flexShrink:0, background:isDone?'rgba(79,140,255,0.1)':'#0d0d0d', border:`1px solid ${isDone?'rgba(79,140,255,0.2)':'#161616'}`, transition:'all 600ms ease' }} />)}
              </div>
              {/* Waveform */}
