...
export function HeroSection() {
  ...
  return (
    <section style={{ position:'relative', zIndex:0, background:'#000000', overflow:'hidden' }}>
      <style>{`
        ...
        @keyframes drag-pulse { 0%,100%{border-color:rgba(255,255,255,0.5)} 50%{border-color:rgba(255,255,255,0.9)} }
      ...
      <div style={{ position:'absolute', top:'-15%', left:'50%', transform:'translateX(-50%)', width:900, height:600, pointerEvents:'none', background:'radial-gradient(ellipse at 50% 0%, rgba(255,255,255,0.11) 0%, transparent 65%)', filter:'blur(60px)', animation:'hero-orb 7s ease-in-out infinite', zIndex:0 }} />
      ...
      <div style={{ position:'relative', zIndex:10, maxWidth:'min(860px,90vw)', margin:'0 auto', padding:'clamp(100px,14vw,160px) clamp(14px,4vw,32px) 0', textAlign:'center' }}>
        ...
        {uploadState === 'empty' && (
          <div style={{ animation:'slide-up 0.7s cubic-bezier(0.22,1,0.36,1) 0.18s both' }}>
            ...
            <div
              ...
              onDrop={handleDrop}
              style={{
                border: isDragging ? '1.5px dashed rgba(255,255,255,0.7)' : '1.5px dashed rgba(255,255,255,0.35)',
                borderRadius: 16, padding: 'clamp(24px,5vw,44px) clamp(18px,4vw,32px)', cursor: 'pointer',
                background: isDragging ? 'rgba(255,255,255,0.06)' : '#0A0A0B',
                backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)',
                boxShadow: isDragging ? '0 0 40px rgba(255,255,255,0.15), 0 12px 36px rgba(0,0,0,0.35)' : '0 12px 40px rgba(0,0,0,0.3)',
                transition: 'all 280ms ease',
                animation: isDragging ? 'drag-pulse 0.8s ease infinite' : 'none',
              }}
            >
              <div style={{ width:48, height:48, borderRadius:'50%', background:'rgba(255,255,255,0.10)', border:'1px solid rgba(255,255,255,0.25)', boxShadow:'none', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
                <Upload size={20} color="#FAFAFA" strokeWidth={1.75} />
              </div>
              ...
              <p style={{ fontSize:13, color:'#A1A1AA', margin:'0 0 12px' }}>
                or <span style={{ color:'#D4D4D8', cursor:'pointer' }}>choose a file</span>
              </p>
            ...
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10, marginTop:20 }}>
              <Link href="/new" style={{ textDecoration:'none' }}>
                <button style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'0 32px', height:52, fontSize:15, fontWeight:600, color:'#09090B', background:'linear-gradient(180deg,#FFFFFF,#E4E4E7)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:12, cursor:'pointer', boxShadow:'0 1px 2px rgba(0,0,0,0.35), 0 6px 18px rgba(255,255,255,0.35), inset 0 1px 0 rgba(255,255,255,0.18)', transition:'all 200ms ease' }}
                  onMouseEnter={e => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 14px 44px rgba(255,255,255,0.6)'; }}
                  onMouseLeave={e => { e.currentTarget.style.transform=''; e.currentTarget.style.boxShadow='0 10px 30px rgba(255,255,255,0.45)'; }}
                >
        ...
        {uploadState === 'uploaded' && uploadedFile && (
          <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'fade-up 0.45s cubic-bezier(0.22,1,0.36,1) both' }}>
            {/* File chip */}
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'#0A0A0B', border:'1px solid #3F3F46', boxShadow:'0 6px 20px rgba(0,0,0,0.35)', borderRadius:10, padding:'9px 14px', margin:'0 auto' }}>
            ...
            {/* AI Prompt box */}
            <div style={{ background:'#0A0A0B', border:`1.5px solid ${promptPhase !== 'idle' ? 'rgba(255,255,255,0.5)' : '#3F3F46'}`, borderRadius:14, overflow:'hidden', boxShadow: promptPhase !== 'idle' ? '0 0 40px rgba(255,255,255,0.12), 0 16px 44px rgba(0,0,0,0.35)' : '0 12px 40px rgba(0,0,0,0.3)', transition:'all 400ms ease', textAlign:'left' }}>
              <div style={{ padding:'16px 18px 12px', minHeight:96 }}>
                <div style={{ fontSize:10, color:'#71717A', letterSpacing:'0.04em', marginBottom:9, display:'flex', alignItems:'center', gap:5 }}>
                  <span style={{ color:'#D4D4D8' }}>✦</span> What should we do with this video?
                </div>
                {promptPhase === 'idle' && !prompt && <span style={{ fontSize:14, color:'#71717A' }}>Describe your edit...</span>}
                {promptPhase === 'typing' && <span style={{ fontSize:14, color:'#D4D4D8', lineHeight:1.65 }}>{demoTyped}<span style={{ display:'inline-block', width:2, height:14, background:'#D4D4D8', marginLeft:1, verticalAlign:'text-bottom', animation:'cursor-blink 0.9s step-end infinite' }} /></span>}
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 14px', borderTop:'1px solid #27272A', background:'#131316' }}>
                <div style={{ display:'flex', gap:4 }}>
                  {['9:16','16:9','1:1'].map(r => <button key={r} onClick={() => setActiveRatio(r)} style={{ padding:'3px 8px', fontSize:11, color: r===activeRatio ? '#FAFAFA' : '#A1A1AA', background: r===activeRatio ? 'rgba(255,255,255,0.1)' : 'transparent', border: r===activeRatio ? '1px solid rgba(255,255,255,0.2)' : '1px solid transparent', borderRadius:5, cursor:'pointer', fontWeight:500 }}>{r}</button>)}
                </div>
                <Link href="/new" style={{ textDecoration:'none' }}>
                  <button style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'8px 16px', fontSize:12, fontWeight:600, color:'#09090B', background:'linear-gradient(180deg,#FFFFFF,#E4E4E7)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:8, cursor:'pointer', boxShadow:'0 4px 14px rgba(255,255,255,0.28)' }}>Edit video <ArrowRight size={12} /></button>
                </Link>
              </div>
            </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:6, justifyContent:'center' }}>
              {PRESETS.map((p, i) => (
                <button key={i} onClick={() => { setPrompt(p.fill); setPromptPhase('idle'); setDemoTyped(''); setDemoSteps([]); clearTimer(); }}
                  style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'6px 13px', fontSize:12, color:'#A1A1AA', background:'transparent', border:'1px solid #27272A', borderRadius:9999, cursor:'pointer', transition:'all 200ms ease' }}
                  onMouseEnter={e => { e.currentTarget.style.color='#FAFAFA'; e.currentTarget.style.borderColor='#27272A'; e.currentTarget.style.background='#131316'; }}
                  onMouseLeave={e => { e.currentTarget.style.color='#A1A1AA'; e.currentTarget.style.borderColor='#27272A'; e.currentTarget.style.background='transparent'; }}
                >
      ...
      {/* ── LOGO STRIP ── */}
      <div style={{ position:'relative', zIndex:10, borderTop:'1px solid #27272A', background:'#0A0A0B', padding:'24px 0 28px', marginTop:60 }}>
...
function EditorMockup({ isDone }: { isDone: boolean }) {
  return (
    <div style={{ position:'relative' }}>
      ...
      <div style={{ position:'absolute', bottom:'-20px', left:'10%', right:'10%', height:100, background:'radial-gradient(ellipse at 50% 100%, rgba(255,255,255,0.18) 0%, transparent 70%)', filter:'blur(30px)', pointerEvents:'none', zIndex:-1 }} />

      <div style={{ borderRadius:14, border:'1px solid #27272A', overflow:'hidden', background:'#0A0A0B', boxShadow:'0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.035)' }}>

        {/* Topbar */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', height:40, background:'#131316', borderBottom:'1px solid #27272A' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            {isDone && <span style={{ fontSize:9, color:'#D4D4D8', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.2)', padding:'2px 7px', borderRadius:9999, fontWeight:600 }}>✦ AI Edit ready</span>}
          </div>
          <div style={{ display:'flex', gap:6 }}>
            <div style={{ width:44, height:22, background:'#1C1C21', borderRadius:5, border:'1px solid #3F3F46' }} />
            <div style={{ width:44, height:22, background:'#1C1C21', borderRadius:5, border:'1px solid #3F3F46' }} />
            <div style={{ width:56, height:22, background:'linear-gradient(180deg,#FFFFFF,#E4E4E7)', borderRadius:5 }} />
          </div>
        </div>
        ...
        <div style={{ display:'flex', height:420 }}>
          ...
          {/* AI Sidebar */}
          <div style={{ width:150, background:'#131316', borderRight:'1px solid #27272A', padding:12, flexShrink:0, display:'flex', flexDirection:'column', gap:2 }}>
            <p style={{ fontSize:9, color:'#D4D4D8', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', margin:'0 0 8px' }}>✦ AI Edit</p>
            {[
              ...
              <div key={i} style={{ padding:'7px 9px', borderRadius:7, background: item.active ? 'rgba(255,255,255,0.12)' : 'transparent', border:`1px solid ${item.active ? 'rgba(255,255,255,0.25)' : 'transparent'}`, marginBottom:2 }}>
                <div style={{ fontSize:10, color: item.active ? '#FAFAFA' : '#71717A', fontWeight: item.active ? 500 : 400, marginBottom:2 }}>{item.label}</div>
                <div style={{ fontSize:8, color:'#D4D4D8', fontWeight:600, letterSpacing:'0.07em' }}>{item.tag}</div>
              </div>
            ))}
            <div style={{ marginTop:'auto', padding:9, borderRadius:7, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.09)' }}>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#0A0A0B' }}>
            ...
            <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
              <div style={{ width:'100%', aspectRatio:'16/9', background:'#000000', borderRadius:10, border:`1px solid ${isDone ? 'rgba(255,255,255,0.35)' : '#27272A'}`, position:'relative', overflow:'hidden', boxShadow: isDone ? '0 0 28px rgba(255,255,255,0.07)' : 'none', transition:'all 600ms ease' }}>
                ...
                <div style={{ position:'absolute', top:10, left:10, display:'flex', alignItems:'center', gap:4, background:'rgba(0,0,0,0.30)', padding:'2px 8px', borderRadius:9999 }}>
                  <span style={{ position:'relative', width:4, height:4, display:'inline-block' }}>
                    <span style={{ position:'absolute', inset:0, borderRadius:'50%', background:'rgba(255,255,255,0.5)', animation:'ping 1.8s ease-out infinite' }} />
                    <span style={{ position:'absolute', inset:0, borderRadius:'50%', background:'#D4D4D8' }} />
                  </span>
                  <span style={{ fontSize:8, color:'#D4D4D8', fontWeight:600 }}>✦ AI Edit</span>
                </div>
                <div style={{ position:'absolute', bottom:12, right:10 }}>
                  <span style={{ fontSize:9, color: isDone ? '#D4D4D8' : '#333', background: isDone ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.45)', border: isDone ? '1px solid rgba(255,255,255,0.25)' : '1px solid #3F3F46', padding:'1px 7px', borderRadius:9999, fontWeight:600, transition:'all 600ms ease' }}>
                    {isDone ? '3:42 · AI Edit' : '42:18 · Raw'}
                  </span>
                </div>
                {isDone && (
                  <div style={{ position:'absolute', bottom:'18%', left:'50%', transform:'translateX(-50%)', background:'rgba(0,0,0,0.82)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:4, padding:'3px 12px', whiteSpace:'nowrap', animation:'edit-ready 0.5s ease both' }}>
            ...
            {/* Timeline */}
            <div style={{ background:'#131316', borderTop:'1px solid #27272A', padding:'10px 16px 14px' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                <span style={{ fontSize:8, color:'#71717A', letterSpacing:'0.04em', textTransform:'uppercase' }}>Timeline</span>
                {isDone && <span style={{ fontSize:8, color:'#D4D4D8', fontWeight:600 }}>✦ 14 AI CUTS APPLIED</span>}
              </div>
              {/* Video track */}
              <div style={{ marginBottom:5 }}>
                ...
                <div style={{ display:'flex', gap:2, alignItems:'center', height:18 }}>
                  {[3,0.3,2,0.3,4,0.3,1.5,0.3,3,0.3,2].map((w,i) => {
                    ...
                    return <div key={i} style={{ height:isCut?8:18, width:w*28, borderRadius:isCut?1:3, flexShrink:0, alignSelf:isCut?'center':'stretch', background:isCut?'transparent':isKept&&isDone?'rgba(255,255,255,0.30)':'#3F3F46', border:`1px solid ${isCut?'transparent':isKept&&isDone?'rgba(255,255,255,0.5)':'#3F3F46'}`, transition:'all 600ms ease', position:'relative' }}>
                      {isCut&&isDone&&<div style={{ position:'absolute', top:-12, left:'50%', transform:'translateX(-50%)', fontSize:6, color:'#D4D4D8', fontWeight:700, whiteSpace:'nowrap' }}>CUT</div>}
                    </div>;
                  })}
                </div>
              </div>
              <div style={{ display:'flex', gap:2, height:10, marginBottom:4 }}>
                {[4,3,3,2.5,2].map((w,i) => <div key={i} style={{ height:10, width:w*28, borderRadius:2, flexShrink:0, background:'#1C1C21', border:'1px solid #27272A' }} />)}
              </div>
              <div style={{ display:'flex', gap:2, height:7, marginBottom:9 }}>
                {[2,1.5,2,1,2.5,1.5].map((w,i) => <div key={i} style={{ height:7, width:w*28, borderRadius:2, flexShrink:0, background:isDone?'rgba(255,255,255,0.1)':'#1C1C21', border:`1px solid ${isDone?'rgba(255,255,255,0.2)':'#1C1C21'}`, transition:'all 600ms ease' }} />)}
              </div>
              {/* Waveform */}
              <div style={{ display:'flex', alignItems:'flex-end', gap:1.5, height:14 }}>
                {BAR_DELAYS.map((delay,i) => <div key={i} style={{ flex:1, height:'100%', borderRadius:2, background:i%3===0?'rgba(255,255,255,0.45)':'rgba(255,255,255,0.14)', animation:`bar-dance 0.65s ease-in-out ${delay} infinite alternate`, transformOrigin:'bottom' }} />)}
              </div>
            </div>
          </div>
        </div>
